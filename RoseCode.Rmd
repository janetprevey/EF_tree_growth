...
all_growth <- growth
growth$HT_cm<-ifelse(all_growth$year%in%c(2016,2017,2018),NA,growth_sub$HT_cm)
```
...
for(i in 1:n){ ## i = individual trees
  ...
  x[t,i]~dnorm(Ex[t],tau_add)  ## process model
...
data <- list(n=length(y),time=time,y=y,NT=10,x_ic=xinit,tau_ic=100,a_obs=1,r_obs=1,a_add=1,r_add=1,a_inc=1,r_inc=1)
